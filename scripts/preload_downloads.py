...
def bootstrap_spine(project_root: Path) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "configs",
        nargs="+",
        help="SPINE config files to load, e.g. infer/2x2/model/model_240819.yaml",
    )
    parser.add_argument(
        "--cache-dir",
        help=(
...
def main() -> int:
    ...
    try:
        ...
        print(
            "ERROR: Could not import SPINE. Source configure.sh or initialize the "
            "spine submodule before running this script.",
            file=sys.stderr,
        )
    ...
    for config in args.configs:
        try:
            ...
            print(f"ERROR: Failed to preload {config}: {exc}", file=sys.stderr)
            return 1
